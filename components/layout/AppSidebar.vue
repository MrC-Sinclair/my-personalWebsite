...
<template>
  <div>
    <aside
      class="bg-surface-light dark:bg-surface-dark hidden border-r border-gray-200 lg:block dark:border-gray-700"
    >
      <nav class="sticky top-20 p-4">
        ...
        <ul class="space-y-1">
          <li v-for="item in items" :key="item.to">
            <NuxtLink
              :to="item.to"
              class="text-text-secondary-light dark:text-text-secondary-dark hover:text-primary-500 block rounded-md px-3 py-1.5 text-sm transition-colors duration-150"
    ...
    >
      <div v-if="isOpen" class="fixed inset-0 z-40 bg-black/50 lg:hidden" @click="emitClose" />
    </Transition>
    ...
    >
      <aside
        v-if="isOpen"
        class="bg-surface-light dark:bg-surface-dark fixed top-0 left-0 z-50 h-full w-72 overflow-y-auto shadow-xl lg:hidden"
      >
        <div
        ...
        <nav class="p-4">
          <ul class="space-y-1">
            <li v-for="item in items" :key="item.to">
              <NuxtLink
                :to="item.to"
                class="text-text-secondary-light dark:text-text-secondary-dark hover:text-primary-500 block rounded-lg px-3 py-2.5 text-sm transition-colors duration-150"
                active-class="text-primary-500 bg-primary-50 dark:bg-primary-950"
                @click="emitClose"
              >
                {{ item.label }}
